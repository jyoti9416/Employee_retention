...
class DatabaseOperation:
    ...
    def database_connection(self,database_name):
        try:
            ...
            self.logger.info("Error while connectiong to the database: %s"%ConnectionError)
        ...
        try:
            ...
            if c.fetchone()[0] == 1:
                ...
                for key in column_name.keys():
                    type = column_name[keys]

                    try:
                        conn.execute("ALTER TABLE "+table_name+f" ADD COLUMN {column_name} {dataType}".format(column_name=key,dataType=type))
                        self.logger.info("ALTER TABLE"+table_name+"ADD COLUMN")

                    except:
                        conn.execute("CREATE TABLE "+table_name+f" {column_name} {dataType}".format(column_name=key,dataType=type))
    ...
    def insert_data(self,database_name,table_name):
        conn = self.database_connection(database_name)
        path = self.data_path
        rejects_path = self.data_path + '_rejects'
        ...
        for file in only_files:
            try:
                ...
                conn.rollback()
                self.logger.exception('Exception raised while inserting data into the table')
                shutil.move(path + '/' + file, rejects_path)
                conn.close()

            conn.close()
            self.logger.info('End of Inserting data into the table')
